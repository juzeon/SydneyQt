 ...
 */
const intToIp = (intIP) => {
  return `${(intIP >> 24) & 255}.${(intIP >> 16) & 255}.${(intIP >> 8) & 255}.${
    intIP & 255
  }`;
};
...
const home = async (pathname) => {
  const baseUrl =
    "https://raw.githubusercontent.com/adams549659584/go-proxy-bingai/master/";
  let url;
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (
      currentUrl.pathname === "/" ||
      currentUrl.pathname.indexOf("/github/") === 0
    ) {
      return home(currentUrl.pathname);
    }
    const targetUrl = new URL(
      SYDNEY_ORIGIN + currentUrl.pathname + currentUrl.search
    );
